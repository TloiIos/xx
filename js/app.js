...
(() => {
  ...
  function renderDevices() {
    ...
    Object.entries(keys).forEach(([keyId, keyData]) => {
      ...
      const hasActiveDevice = Object.values(devices).some(v => v === true);
      if (hasActiveDevice) activeKeys++;
      
      Object.entries(devices).forEach(([uid, active]) => {
        if (active) {
          const record = {
            uid,
            keyId: keyData.key || keyId,
            keyData: keyData,
            created: keyData.createdAt || 0,
            active: true
          };
          if (!deviceMap.has(uid) || record.created > deviceMap.get(uid).created) {
            deviceMap.set(uid, record);
          }
          if (record.created > latestTime) {
            latestTime = record.created;
            latestUid = uid;
          }
        }
      });
    ...
    const statKeysInUse = $("#statKeysInUse");
    if (statKeysInUse) statKeysInUse.textContent = activeKeys;
    ...
    if (latestUid) {
      if (latestUidDisplay) latestUidDisplay.textContent = latestUid;
      if (latestUidTime) latestUidTime.textContent = fmtDate(latestTime);
    } else {
      if (latestUidDisplay) latestUidDisplay.textContent = '—';
  ...
  function renderUidTable(deviceMap) {
    ...
    if (search) {
      list = list.filter(d => 
        ...
        d.keyId.toLowerCase().includes(search)
      );
    }
    ...
    if (uidSortKey) {
      list.sort((a, b) => {
        ...
        if (uidSortKey === 'key') { va = a.keyId; vb = b.keyId; }
        if (uidSortKey === 'created') { va = a.created; vb = b.created; }
        va = va ?? 0; vb = vb ?? 0;
        const r = typeof va === 'string' ? va.localeCompare(vb) : va - vb;
        return uidSortDir === 'asc' ? r : -r;
      });
    }
    ...
    const subtitle = document.getElementById('devicesSubtitle');
    if (subtitle) subtitle.textContent = `${list.length} UID đang hoạt động`;
    ...
    body.innerHTML = slice.map(d => `
      <div class="dg-row dg-row--keys" style="grid-template-columns:1.5fr 1.2fr 1.2fr 0.8fr 0.6fr;" data-uid="${esc(d.uid)}">
        ...
        <span class="dg-cell dg-id"><code class="key-code" data-copy="${esc(d.keyId)}">${esc(d.keyId)}</code></span>
        <span class="dg-cell dg-date">${fmtDate(d.created)}</span>
    ...
    if (uidPag) {
      ...
      for (let i = 1; i <= pages; i++) {
        if (pages > 7 && i > 2 && i < pages - 1 && Math.abs(i - uidPage) > 1) {
          if (!html.endsWith("…</span>")) html += `<span class="page-ellipsis">…</span>`;
          continue;
        }
  ...
  document.querySelectorAll('[data-sort-uid]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (uidSortKey === key) {
        uidSortDir = uidSortDir === 'asc' ? 'desc' : 'asc';
      } else {
        uidSortKey = key;
        uidSortDir = 'desc';
      }
  ...
  document.addEventListener('click', async (e) => {
    const removeUidEl = e.target.closest('[data-remove-uid]');
    if (removeUidEl) {
      const uid = removeUidEl.dataset.removeUid;
      if (!confirm(`Xoá UID "${uid}" khỏi tất cả key?`)) return;
      
      try {
        let found = false;
        for (const [keyId, keyData] of Object.entries(keys)) {
          if (keyData.devices && keyData.devices[uid]) {
            const newDevices = { ...keyData.devices };
            delete newDevices[uid];
            await KeyAPI.updateKey(keyId, { devices: newDevices });
            keys[keyId].devices = newDevices;
            found = true;
        ...
        if (found) {
          renderAll();
          toast('success', 'Đã xoá UID', `UID ${uid} đã được xoá khỏi hệ thống.`);
        } else {
          toast('warning', 'Không tìm thấy', `UID ${uid} không tồn tại trong hệ thống.`);
        }
      } catch (err) {
        toast('danger', 'Lỗi', err.message);
      }
  ...
  // Gán UID form
  const assignForm = document.getElementById('assignUidForm');
  if (assignForm) {
    assignForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const keyInput = document.getElementById('assignKeyInput');
      const uidInput = document.getElementById('assignUidInput');
      const keyVal = keyInput.value.trim();
      const uidVal = uidInput.value.trim();
      const btn = document.getElementById('assignUidBtn');

      if (!keyVal || !uidVal) {
        toast('warning', 'Thiếu thông tin', 'Vui lòng nhập cả Key và UID.');
        return;
      }
      ...
      try {
        ...
        for (const [id, data] of Object.entries(keys)) {
          if (data.devices && data.devices[uidVal] && id !== foundId) {
            uidExistsInOtherKey = true;
            break;
          }
        }

        if (uidExistsInOtherKey) {
          toast('warning', 'UID đã tồn tại', `UID ${uidVal} đang được sử dụng ở key khác.`);
          btn.classList.remove('is-loading');
          btn.disabled = false;
          return;
        }
        ...
        if (activeCount >= maxDevices) {
          toast('warning', 'Đã đầy thiết bị', `Key chỉ hỗ trợ tối đa ${maxDevices} thiết bị. Đã có ${activeCount} thiết bị.`);
          btn.classList.remove('is-loading');
          btn.disabled = false;
          return;
        }
        ...
        keys[foundId].devices = newDevices;

        renderAll();
        keyInput.value = '';
        uidInput.value = '';
        toast('success', 'Đã gán UID', `UID ${uidVal} được gán cho key ${keyVal}. Còn ${maxDevices - activeCount - 1} thiết bị trống.`);
      } catch (err) {
        toast('danger', 'Lỗi gán UID', err.message);
      } finally {
        btn.classList.remove('is-loading');
        btn.disabled = false;
      }
